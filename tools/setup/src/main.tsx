...
function createArticleSlug() {
  return `article-${globalThis.crypto?.randomUUID?.().slice(0, 8) ?? Math.random().toString(36).slice(2, 10)}`;
}
...
function Field({ label, value, onChange, multiline = false, type = "text", invalid = false }: { label: string; value: string; onChange: (value: string) => void; multiline?: boolean; type?: string; invalid?: boolean }) {
  return <label className="field"><span>{label}</span>{multiline ? <textarea value={value} onChange={(event) => onChange(event.target.value)} rows={4} aria-invalid={invalid || undefined} /> : <input type={type} value={value} onChange={(event) => onChange(event.target.value)} aria-invalid={invalid || undefined} />}</label>;
}
...
function ArticleEditor({ article, onChange, onSave, busy, creating = false, onCancel }: { article: Article; onChange: <K extends keyof Article>(key: K, value: Article[K]) => void; onSave: () => void; busy: boolean; creating?: boolean; onCancel?: () => void }) {
  const slugError = !article.slug ? "URL 标识不能为空。" : articleSlugPattern.test(article.slug) ? "" : "仅支持小写英文、数字和短横线，且不能以短横线开头或结尾。";
  return <article className="article-config-row"><header><div><span>{creating ? "新建 Markdown 草稿" : `/${article.slug}`}</span><strong>{article.title || "新文章"}</strong></div><div className="article-form-actions">{creating && onCancel ? <button className="quiet-button" type="button" disabled={busy} onClick={onCancel}>取消</button> : null}<button className="quiet-button" type="button" disabled={busy || (creating && Boolean(slugError))} onClick={onSave}>{creating ? "创建文章" : "保存文章"}</button></div></header><div className="article-config-fields">{creating ? <div className="article-slug-field"><Field label="URL 标识（自动生成，可修改）" value={article.slug} invalid={Boolean(slugError)} onChange={(value) => onChange("slug", value.toLowerCase().trim())} />{slugError ? <p className="field-error" role="alert">{slugError}</p> : null}</div> : null}<Field label="标题" value={article.title} onChange={(value) => onChange("title", value)} /><Field label="发布日期" value={article.publishedAt} type="date" onChange={(value) => onChange("publishedAt", value)} /><Field label="阅读时长" value={article.readingTime} onChange={(value) => onChange("readingTime", value)} /><label className="field checkbox-field"><span>首页精选</span><input type="checkbox" checked={article.featured} onChange={(event) => onChange("featured", event.target.checked)} /></label><Field label="标签（用逗号分隔）" value={article.tags.join(", ")} onChange={(value) => onChange("tags", value.split(",").map((item) => item.trim()).filter(Boolean))} /><Field label="摘要" value={article.summary} multiline onChange={(value) => onChange("summary", value)} /></div></article>;
}
...
function App() {
  ...
  const updateNewArticle = <K extends keyof Article>(key: K, value: Article[K]) => setNewArticle((current) => current ? { ...current, [key]: value } : current);
  const startNewArticle = () => setNewArticle({ slug: createArticleSlug(), title: "新文章", summary: "补充这篇文章的摘要。", publishedAt: new Date().toISOString().slice(0, 10), readingTime: "5 min", tags: [], featured: false });
  ...
  return <main className="setup-shell">
    ...
    <section className="editor">
      ...
      {activeTab === "content" ? <section className="editor-section"><div className="section-heading"><h3>内容页面</h3><span>文章正文继续由 Markdown 管理</span></div><Field label="文章页说明" value={draft.site.pages.articlesDescription} multiline onChange={(value) => updateSite("pages", { ...draft.site.pages, articlesDescription: value })} /><Field label="项目页说明" value={draft.site.pages.projectsDescription} multiline onChange={(value) => updateSite("pages", { ...draft.site.pages, projectsDescription: value })} /><Field label="文章署名" value={draft.site.pages.articleAuthor} onChange={(value) => updateSite("pages", { ...draft.site.pages, articleAuthor: value })} /><div className="section-heading article-list-heading"><div><h3>文章列表</h3><span>可新建 Markdown 草稿；修改已有文章的摘要、日期、标签和精选状态会直接更新对应文件。</span></div><button className="quiet-button" type="button" disabled={busy || Boolean(newArticle)} onClick={startNewArticle}>新增文章</button></div><div className="article-editor">{newArticle ? <ArticleEditor article={newArticle} busy={busy} creating onCancel={() => setNewArticle(undefined)} onChange={updateNewArticle} onSave={() => void run(() => request<State>("/articles", { article: newArticle }), `已创建「${newArticle.title}」，请继续编辑生成的 Markdown 正文。`).then((created) => { if (created) setNewArticle(undefined); })} /> : null}{articleDrafts.map((article) => <ArticleEditor key={article.slug} article={article} busy={busy} onChange={(key, value) => updateArticle(article.slug, key, value)} onSave={() => void run(() => request<State>(`/articles/${article.slug}`, { article }), `已保存「${article.title}」。`)} />)}</div></section> : null}
